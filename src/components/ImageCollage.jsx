...
const ImageCollage = () => {
    ...
    return (
        <section ref={component} className="relative w-full overflow-hidden pt-6 sm:pt-8  px-4 sm:px-6">
            <div className="max-w-xs sm:max-w-lg md:max-w-2xl lg:max-w-4xl mx-auto h-full  flex justify-center items-center">

                {/* --- Background SVG Shape --- */}
                <img src={blueDoodle} alt="" className="absolute left-2 sm:left-4 lg:left-1/21 top-0 w-1/4 sm:w-1/3 h-auto text-blue-400 z-0" />
                ...
                <div ref={imagesRef} className="relative w-full h-[50vh] lg:h-[80vh]">
                    {imageData.map((image, index) => (
                        ...
                        >
                            ...
                            {image.tag && (
                                <div className={`absolute bottom-2 sm:bottom-3 lg:bottom-4 right-2 sm:right-3 lg:right-4 text-black text-xs sm:text-sm font-semibold p-1.5 sm:p-2 rounded-md sm:rounded-lg ${image.tagColor}`}>
                                    <span className="hidden sm:inline">{image.tag}</span>
                                    <span className="sm:hidden">
                                        {image.tag.length > 15 ? image.tag.substring(0, 15) + '...' : image.tag}
                                    </span>
                                </div>
                            )}
                        </div>
            ...
            <p className='dm-sans font-light text-black text-center max-w-xs sm:max-w-lg md:max-w-xl lg:max-w-2xl mx-auto text-sm sm:text-base md:text-lg lg:text-xl lg:pt-10 px-2 sm:px-0 pb-8'>
                We are a live entertainment company focused on producing concerts, public shows, and large-scale events. From securing talent to full-scale production, we bring vision, sound, and crowd energy together to create experiences that audiences remember.
            </p>
        </section>
    );
};
